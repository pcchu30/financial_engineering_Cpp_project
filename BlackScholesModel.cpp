...
BlackScholesModel::BlackScholesModel() :
    drift(0.0), stockPrice(0.0),
    volatility(0.0), riskFreeRate(0.0),
    date(0.0) { 
}

/*
 * Create a risk neutral price path according to the model parameters
 */
...
/*
 * Create a price path according to the model parameters
 */
...
/*
 * Private: Create a price path according to the model parameters
 */
...
void testRiskNeutralPricePath() {
    ...
    for (int i=0; i<nPaths; i++) {
        vector<double> path = bsm.generateRiskNeutralPricePath(
                                    maturity, nsteps );
